...
#all necessary matrix or vector operations can be found at https://docs.blender.org/api/current/mathutils.html
    ...
    def createView():
        ...
        Z = eyePosition - centerPosition
        #Z = mathutils.Vector((0.0, 0.0, 0.0))
        euler = mathutils.Euler((0.0, 0.0, math.radians(13.0)), 'XYZ')
        Z.rotate(euler)
        Z.normalize()
    ...
    def createTransformation(object, view):
        ...
        #######################
        if (objectNumber % 2 == 0):
            transformation = mathutils.Matrix.Translation((objectNumber-3.5, 0, 0))
        else:
            transformation = mathutils.Matrix.Translation((objectNumber-3.5, 0, 0.5))
        
        transformation = view @ transformation
        rotation = mathutils.Matrix.Rotation(radians(90), 4, 'X')
